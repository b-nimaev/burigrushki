<?php
/*
Plugin Name: Сбербанк ID
Description: Единый аккаунт для доступа к сервисам Сбербанка и наших партнеров! Для заключения договора оставьте  <a href="https://www.sberbank.ru/ru/person/dist_services/sberbankid/forbusiness">заявку</a>.
Author: RBSPayment
...
class sberbank_id  {
    ...
    public function viewAdminPageSettings() {
        if(isset($_POST['sberbank_id'])) {
          update_option('sberbank_id', $_POST['sberbank_id']);
        }
        if(isset($_FILES['sertificate']) && $_FILES['sertificate']['size'] > 0) {
          ...
          if($file_parts['extension'] == 'p12') {       
            $this->sberbank_id->setSertificatePassword($_POST['sberbank_id']['sert_password']);
    ...
    public function auth_request() {
      ...
      if(isset($_REQUEST['error'])) {
        ...
        $auth = wp_authenticate( $_POST['email'], $_POST['password'] );
        if(is_wp_error( $auth )) {
            ...
            'auth_error' => 1,
            'sberbank_user' => [
              'phone' => $_POST['sberbank_user_phone'],
              'sub' => $_POST['sberbank_user_sub'],
              'aud' => $_POST['sberbank_user_aud'],
              'family_name' => $_POST['sberbank_user_family_name'],
              'given_name' => $_POST['sberbank_user_given_name'],
              'middle_name' => $_POST['sberbank_user_middle_name'],
            ],
          ));
        } else {
    ...
    public function wp_show_login_form($data) {
      ...
      $data['entry_title'] = 'Вы успешно вошли по Сбербанк ID.<br>Мы нашли вашу учетную запись на сайте<br>';
      $data['entry_footer'] = 'Вы можете привязать ee к аккаунту Сбербанк ID. Для этого подтвердите эту учетную запись <b>паролем</b>';
